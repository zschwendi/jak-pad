...
const char* phase_name(jak2_fr3::Phase phase) {
  switch (phase) {
    case jak2_fr3::Phase::loading_configuration:
      return "configuration";
    case jak2_fr3::Phase::reading_archives:
      return "archives";
    case jak2_fr3::Phase::linking_objects:
      return "linking";
    case jak2_fr3::Phase::extracting_intermediates:
      return "intermediates";
    case jak2_fr3::Phase::extracting_levels:
      return "levels";
  }
  return "unknown";
}
...
int main(int argc, char** argv) {
  if (argc != 4) {
    std::cerr << "usage: jak2-fr3-prepare PROJECT_ROOT EXTRACTED_ISO_ROOT WORK_ROOT\n";
    return 2;
  }
  ...
  jak2_fr3::Options options;
  options.should_cancel = [] { return g_cancelled.load(std::memory_order_relaxed); };
  options.report_progress = [](const jak2_fr3::Progress& progress) {
  ...
  const auto result = jak2_fr3::prepare(std::filesystem::path(argv[1]),
                                        std::filesystem::path(argv[2]),
                                        std::filesystem::path(argv[3]), options);
  if (!result) {
    std::cerr << jak2_fr3::error_code_name(result.error().code) << ": "
