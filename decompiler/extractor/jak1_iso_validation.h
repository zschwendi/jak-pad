...
/// digest per file in the reader's depth-first, on-disc record order; the same traversal is used to
/// associate a root executable with its digest without rereading the extracted file.
ValidationResult<RevisionMatch> validate_extracted_layout(const IsoFile& layout);

/// Atomically add the desktop-compatible buildinfo.json phase checkpoint to a validated staging
/// directory. Existing checkpoint files are never overwritten.
