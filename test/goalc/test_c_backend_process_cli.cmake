...
set(GENERATED_C "${OUTPUT_DIR}/proc.c")
set(GENERATED_H "${OUTPUT_DIR}/proc_generated.h")
set(REPORT "${OUTPUT_DIR}/proc.report.txt")
set(TEST_BINARY "${OUTPUT_DIR}/process_c_backend_test")
...
execute_process(
  COMMAND "${GOALC_CBACKEND}"
          ...
          "${PROJECT_ROOT}/test/goalc/aot/process_test.gc"
  RESULT_VARIABLE EMIT_RESULT)
if(NOT EMIT_RESULT EQUAL 0)
  message(FATAL_ERROR "goalc-cbackend failed on the process-register fixture")
endif()

file(READ "${REPORT}" REPORT_TEXT)
if(REPORT_TEXT MATCHES "FAILED")
  message(FATAL_ERROR "goalc-cbackend could not translate the process-register fixture:\n${REPORT_TEXT}")
endif()
...
execute_process(
  COMMAND "${C_COMPILER}"
          ...
          "${PROJECT_ROOT}/test/goalc/aot/process_c_backend_test.c"
...
if(NOT HOST_RUN_RESULT EQUAL 0)
  message(FATAL_ERROR "The process register did not behave correctly in the generated C")
endif()
