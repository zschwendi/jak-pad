...
Val* Compiler::generate_inspector_for_structure_type(const goos::Object& form,
  ...
  constraint.desired_register =
      emitter::get_register_info(m_instr_set).get_gpr_arg_reg(0);  // to the first argument
...
Val* Compiler::generate_inspector_for_bitfield_type(const goos::Object& form,
  ...
  if (bitfield_128) {
    constraint.desired_register =
        emitter::get_register_info(m_instr_set).get_xmm_arg_reg(0);  // to the first argument
  } else {
    constraint.desired_register =
        emitter::get_register_info(m_instr_set).get_gpr_arg_reg(0);  // to the first argument
  }
  ...
  if (bitfield_128) {
    method_env->emit_ir<IR_Return>(form, method_env->make_ireg(input->type(), RegClass::INT_128),
                                   input, emitter::get_register_info(m_instr_set).get_gpr_ret_reg());
  } else {
    method_env->emit_ir<IR_Return>(form, method_env->make_gpr(input->type()), input,
                                   emitter::get_register_info(m_instr_set).get_gpr_ret_reg());
  }
...
Val* Compiler::compile_defmethod(const goos::Object& form, const goos::Object& _rest, Env* env) {
  ...
  if (behavior) {
    ...
    constr.desired_register = emitter::get_register_info(m_instr_set).get_process_reg();
  ...
  if (new_func_env->is_asm_func) {
    ...
    RegVal* final_result;
    emitter::Register ret_hw_reg = emitter::get_register_info(m_instr_set).get_gpr_ret_reg();
    if (m_ts.lookup_type(result->type())->get_load_size() == 16) {
      ret_hw_reg = emitter::get_register_info(m_instr_set).get_xmm_ret_reg();
