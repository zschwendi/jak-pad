...
CallingConvention get_function_calling_convention(const TypeSpec& function_type,
  ...
  int gpr_idx = 0;
  int xmm_idx = 0;
  const auto& register_info = emitter::get_register_info(instruction_set);

  CallingConvention cc;

  if (function_type.arg_count() == 2 && function_type.get_arg(0).print() == "_varargs_") {
    for (int i = 0; i < 8; i++) {
      cc.arg_regs.push_back(register_info.get_gpr_arg_reg(gpr_idx++));
    }
  } else {
    for (int i = 0; i < (int)function_type.arg_count() - 1; i++) {
      ...
      if (dynamic_cast<const ValueType*>(info) && load_size == 16) {
        cc.arg_regs.push_back(register_info.get_xmm_arg_reg(xmm_idx++));
      } else {
        cc.arg_regs.push_back(register_info.get_gpr_arg_reg(gpr_idx++));
      }
    }
  ...
  if (function_type.last_arg() != TypeSpec("none")) {
    if (type_system.get_load_size_allow_partial_def(function_type.last_arg()) == 16) {
      cc.return_reg = register_info.get_xmm_ret_reg();
    } else {
      cc.return_reg = register_info.get_gpr_ret_reg();
    }
  }
...
std::vector<emitter::Register> get_arg_registers(const TypeSystem& type_system,
  ...
  for (auto& type : arg_types) {
    ...
    if (load_size == 16) {
      result.push_back(register_info.get_xmm_arg_reg(xmm_idx++));
    } else {
      result.push_back(register_info.get_gpr_arg_reg(gpr_idx++));
    }
  }
  return result;
}
