#include "Jak2SourceObjectPack.h"
...
bool valid_source_path(std::string_view source) {
  if (source.starts_with("goal_src/jak2/") && source.ends_with(".gc")) {
    return true;
  }
  return source == "goal_src/jak1/pc/pckernel-h.gc" ||
         source == "goal_src/jak1/pc/debug/pc-debug-common.gc" ||
         source == "goal_src/jak1/pc/pckernel-common.gc";
}

bool valid_source_graph(std::span<const std::string> sources) {
  if (sources.size() != kExpectedObjectCount) {
    return false;
  }
  std::set<std::string> unique;
  return std::all_of(sources.begin(), sources.end(), [&](const auto& source) {
    return valid_source_path(source) && source.find_first_of("\t\r\n") == std::string::npos &&
           unique.emplace(source).second;
  });
}
...
Result<Summary> validate(const fs::path& root,
                         std::span<const std::string> expected_source_files,
                         const Options& options) {
  try {
    if (!valid_options(options, expected_source_files) || !root.is_absolute()) {
    ...
    if (manifest.value().entries.size() != expected_source_files.size()) {
      return Result<Summary>::failure(
          make_error(ErrorCode::source_graph_mismatch,
                     "The source-object pack does not match the Jak II GROUP:all-code graph."));
    }
