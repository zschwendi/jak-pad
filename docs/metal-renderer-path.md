...
- **Implemented** (stage 4, DMA chain consumption and first bucket renderers):
  ...
  - Verified by `metal-proof` (now 163 checks without arguments, 168 with an `.fr3`;
...
- **Implemented** (captured-chain replay, version 2 captures): `metal-proof --replay
  ...
  - Results on the runtime track's title-screen captures (Jak 1, title + Sandover
    ...
    - frame 838/839/840 (~154 kB of payload each): the sky renders for real - the
      ...
      992 bytes of tfrag-trans inside the sky-blend bucket. (merc is drawn now; see the
      merc entry below for what those frames render today.)
    - frame 100 (14 kB): originally rendered black - its content is merc models plus a
      ...
      sky texture that frame. With the merc renderer (below) it draws Daxter.
...
- **Implemented** (stage 6 part 1, the merc renderer): `metal_merc.{h,mm}` +
  ...
  - On the captured frames (`--replay`, with the level art named):
    - **frame 838/839/840 now render the Jak & Daxter title logo** - `logo-english-lod0` from
...
## 5. Next target: Ocean (scoped, not started)

Merc2 is done (see §4). Ocean is the biggest payload carrier left in the captured title
frames (~121 kB/frame). Scoped from the GL sources; it is *Investigating* only in the
sense that the work below is not started - the data flow itself is settled.

**Ocean.** It has *no* Loader or level-data dependency: nothing under
