...
/*!
 * @file ShadowVu.h
 * VU1 emulation for the Jak 1 shadow renderer, with no graphics API in it.
 *
 * Shadow_PS2.cpp is a ~1.9k line transcription of the VU1 microprogram that
 * builds the shadow volume. It touches nothing but VU registers, VU data
 * memory and the CPU-side vertex/index buffers, so every backend can run the
 * exact same code: this class owns that state and those functions, and each
 * backend's shadow renderer derives from it and supplies only the drawing.
 *
 * Same pattern (and the same reason) as ocean/OceanVu.h.
 */
...
class ShadowVu {
 ...
 protected:
  ...
  // the GIF "kick": turns the VU's output quadwords into the vertex and index
  // buffers above. Pure CPU work, shared by every backend.
  void xgkick(u16 imm);
