...
bool all_finite(const SpriteGlowData& data) {
  for (float component : data.pos) {
    if (!std::isfinite(component)) {
      return false;
    }
  }
  for (float component : data.color) {
    if (!std::isfinite(component)) {
      return false;
    }
  }
  return std::isfinite(data.size_x) && std::isfinite(data.size_probe) &&
         std::isfinite(data.z_offset) && std::isfinite(data.rot_angle) &&
         std::isfinite(data.size_y) && std::isfinite(data.fade_a) &&
         std::isfinite(data.fade_b);
}

bool all_finite(const SpriteGlowConsts& consts) {
  for (const auto& row : consts.camera) {
    if (!all_finite(row)) {
      return false;
    }
  }
  for (const auto& row : consts.perspective) {
    if (!all_finite(row)) {
      return false;
    }
  }
...
bool all_finite(const SpriteGlowOutput& output) {
...
bool glow_math(const SpriteGlowConsts* consts,
  ...
  memcpy(&result.adgif, adgif_data, 5 * 16);
  ...
  p0.y() *= perspective_q;
  p0.z() *= perspective_q;
  result.perspective_q = perspective_q;
  ...
  result.second_clear_pos[0] =
      Vector4f(p0.x() - vf02.x(), p0.y() - vf02.x(), p0.z(), p0.w());
  result.second_clear_pos[1] =
      Vector4f(p0.x() + vf02.x(), p0.y() + vf02.x(), p0.z(), p0.w());
  ...
  vf18 = vf20_pos - vf18;
  vf18 += vf02.x();
  result.offscreen_uv[0] = vf17;
  result.offscreen_uv[1] = vf18;

  result.first_clear_pos[0] =
      Vector4f(vf20_pos.x() - vf02.x() - 1, vf20_pos.y() - vf02.x() - 1, 0xffffff, p0.w());
  result.first_clear_pos[1] =
      Vector4f(vf20_pos.x() + vf02.x() + 1, vf20_pos.y() + vf02.x() + 1, 0xffffff, p0.w());
