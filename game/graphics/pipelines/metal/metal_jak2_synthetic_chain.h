...
constexpr std::size_t kJak2SyntheticDeferredBucket =
    static_cast<std::size_t>(jak2::BucketId::SHADOW);
...
// leaves renderer-owned buckets on their valid empty path, gives one DeferredSkip bucket a
// 16-byte marker, and gives SCREEN_FILTER four VIF NOPs. It contains no game data and
// intentionally encodes no draw.
inline std::vector<u8> make_jak2_synthetic_metal_chain() {
  ...
  put_jak2_synthetic_tag(chain, kJak2SyntheticDeferredBucket * 16, DmaTag::Kind::NEXT, 0,
  ...
  put_jak2_synthetic_tag(chain, kDeferredPayloadOffset + 32, DmaTag::Kind::NEXT, 0,
                         static_cast<u32>((kJak2SyntheticDeferredBucket + 1) * 16));
