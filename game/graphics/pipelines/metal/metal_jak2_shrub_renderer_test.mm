...
bool write_synthetic_fr3(const std::filesystem::path& path) {
  ...
  tree.packed_vertices.matrices.push_back(identity);
  ...
  tree.packed_vertices.instance_groups.push_back({0, 0, 8, 0});
  tree.packed_vertices.total_vertex_count = 8;
  tree.indices = {0, 1, 2, 3, 4, 5, 6, 7};
  tree.static_draws.push_back(shrub_draw(0, 0));
  tree.static_draws.push_back(shrub_draw(4, 1));
  ...
  level.shrub_trees.push_back(std::move(tree));
...
int main() {
  @autoreleasepool {
    ...
    check(hidden.completed && hidden.finished_bucket && hidden.renderer.trees_rendered == 1 &&
              hidden.renderer.draws == 1 && hidden.renderer.triangles == 2 &&
              hidden.draw_calls == 1 && hidden.triangles == 2,
          "the 160-byte mask hides one named proto and draws the other exactly once");
    ...
    const auto zero = render_chain(device, queue, &pso_cache, &sampler_cache, &texture_pool,
                                   &renderer, zero_chain);
    check(zero.completed && zero.finished_bucket && zero.renderer.trees_rendered == 1 &&
              zero.renderer.draws == 2 && zero.renderer.triangles == 4 && zero.draw_calls == 2 &&
              zero.triangles == 4 && zero.background.unexpected_dma == 0,
          "a zero-qword mask draws both named proto regions");
    ...
    auto& first_draw = level->level->shrub_trees.at(0).static_draws.at(0);
    first_draw.proto_idx = 2;
    const auto bad_index = render_chain(device, queue, &pso_cache, &sampler_cache, &texture_pool,
                                        &renderer, zero_chain);
    first_draw.proto_idx = 0;
    check(bad_index.completed && bad_index.finished_bucket && bad_index.draw_calls == 0 &&
              bad_index.triangles == 0 && bad_index.background.unexpected_dma == 1,
          "an out-of-range proto_idx fails the whole bucket closed before drawing");
