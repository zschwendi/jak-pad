 ...
 *
 * Not ported yet, counted and logged rather than dropped:
 *  - the distorter's *drawing* (heat shimmer). Its DMA is walked exactly like
 *    the GL renderer's distort_dma, so the chain stays in sync and the sprite
 *    count is reported; the effect needs a framebuffer snapshot pass.
 *  - Jak 2/3 paths (render_jak2, glow). This renderer is Jak 1 only, matching
 *    the rest of the Metal bucket table.
 */
...
class MetalSpriteRenderer : public MetalBucketRenderer {
 public:
  ...
  struct Stats {
    ...
    int draw_calls = 0;
    int triangles = 0;
    int distort_sprites = 0;  // DMA walked, drawing not ported
