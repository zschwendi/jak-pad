...
constexpr std::size_t kJak2BombAnimatedTextureSlot = 15;
...
class Jak2Opcode27SkullGemExecutor {
 public:
  ...
  struct Stats {
    ...
    u64 bomb_preparations = 0;
    u64 bomb_publications = 0;
  };

  struct PreparedFixedOutput {
    u32 destination_tbp = 0;
    u16 width = 0;
    u16 height = 0;
    std::vector<u8> rgba;
  ...
  using PreparedSecurityOutput = PreparedFixedOutput;
  ...
  struct PreparedBomb : PreparedFixedOutput {};
  ...
  bool prepare_bomb(const Jak2Opcode28BombPlan& plan,
                    const tfrag3::Level& game_level,
                    PreparedBomb* out);
  bool publish_bomb(const PreparedBomb& prepared);
  void detach_pool();
 ...
 private:
  ...
  bool publish_fixed_output(const PreparedFixedOutput& prepared,
                            std::size_t slot,
                            const char* label,
  ...
  std::unique_ptr<MetalPoolTexture> m_bomb_publication;
