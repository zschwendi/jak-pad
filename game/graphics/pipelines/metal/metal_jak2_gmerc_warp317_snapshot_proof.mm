...
constexpr u8 kLaterR = 20;
constexpr u8 kLaterG = 30;
constexpr u8 kLaterB = 40;
constexpr u8 kLaterA = 255;
...
constexpr Pixel kLaterPixel = {kLaterR, kLaterG, kLaterB, kLaterA};
...
id<MTLRenderCommandEncoder> begin_pass(id<MTLCommandBuffer> commands,
  ...
  pass.colorAttachments[0].clearColor =
      MTLClearColorMake(kLaterR / 255.f, kLaterG / 255.f, kLaterB / 255.f, 1.f);
...
RenderResult render(id<MTLDevice> device,
  ...
  if (publisher) {
    result.published = publisher->publish(&state, context);
  }
...
bool publish_only(id<MTLDevice> device,
  ...
  state.version = GameVersion::Jak2;
  state.texture_pool = texture_pool;
  const bool published = publisher->publish(&state, context);
  [context.enc endEncoding];
  [commands commit];
  [commands waitUntilCompleted];
...
int changed_pixel_count(const RenderResult& result) {
  ...
  for (int y = 0; y < kTargetSize; y++) {
    for (int x = 0; x < kTargetSize; x++) {
      count += !(pixel_at(result.pixels, kTargetSize, x, y) == kLaterPixel);
    }
  }
  return count;
}
  ...
  @autoreleasepool {
    ...
    check(policy.at(static_cast<std::size_t>(jak2::BucketId::TEX_ALL_WARP)).behavior ==
              ...
              policy.at(static_cast<std::size_t>(jak2::BucketId::GMERC_WARP)).behavior ==
                  metal_renderer::Jak2MetalBucketBehavior::DeferredSkip,
          "buckets 316 and 317 remain deferred during the private proof");
    ...
    check(near(edge_center, {90, 19, 24, 129}, 3) && edge_center.r > edge_center.b + 40,
          "same-command-buffer snapshot precedes later target pixels and forces edge clamp");
    ...
    check(exact_draw(fog, fog_chain) && near(fog_center, {30, 75, 120, 129}, 3) &&
    ...
    std::printf("PASS: Jak II GMERC_WARP snapshot and Generic2 proof stayed disconnected\n");
    return 0;
  }
}
