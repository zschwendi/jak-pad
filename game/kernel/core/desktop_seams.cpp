...
u64 portable_file_stream_open(u64 stream_offset, u64 name_offset, u64 mode_offset) {
  auto* stream = Ptr<FileStream>((u32)stream_offset).c();
  stream->flags = 0;
  stream->mode = (u32)mode_offset;
  stream->name = (u32)name_offset;

  const char* mode = goal_game_symbol_name((u32)mode_offset);
  int flags = SCE_TRUNC | SCE_CREAT | SCE_WRONLY;
  if (!std::strcmp(mode, "read")) {
    flags = SCE_RDONLY;
  } else if (!std::strcmp(mode, "append")) {
    flags = SCE_CREAT | SCE_WRONLY;
  }
  stream->file = ee::sceOpen(Ptr<String>((u32)name_offset)->data(), flags);
  return stream_offset;
}

u64 portable_file_stream_close(u64 stream_offset) {
  auto* stream = Ptr<FileStream>((u32)stream_offset).c();
...
s64 portable_file_stream_length(u64 stream_offset) {
  auto* stream = Ptr<FileStream>((u32)stream_offset).c();
  if ((stream->flags & 1) || stream->file < 0) {
    return 0;
  }
...
s64 portable_file_stream_seek(u64 stream_offset, s32 offset, s32 where) {
  auto* stream = Ptr<FileStream>((u32)stream_offset).c();
  if ((stream->flags & 1) || stream->file < 0) {
    return -1;
  }
...
s64 portable_file_stream_read(u64 stream_offset, u64 buffer_offset, s32 size) {
  auto* stream = Ptr<FileStream>((u32)stream_offset).c();
  if ((stream->flags & 1) || stream->file < 0) {
    return -1;
  }
  const s32 result = ee::sceRead(stream->file, Ptr<u8>((u32)buffer_offset).c(), size);
  if (result < 0) {
    stream->flags |= 1;
  }
  return result;
...
s64 portable_file_stream_write(u64 stream_offset, u64 buffer_offset, s32 size) {
  auto* stream = Ptr<FileStream>((u32)stream_offset).c();
  if ((stream->flags & 1) || stream->file < 0) {
    return -1;
  }
  const s32 result = ee::sceWrite(stream->file, Ptr<u8>((u32)buffer_offset).c(), size);
  if (result < 0) {
    stream->flags |= 1;
  }
  return result;
